...
plan tests => repeat_each() * (6 * 3 + 5 * 9);
...
=== TEST 12: file seek to a fixed postition and result in the final offset is negative
--- main_config
thread_pool default threads=2 max_queue=10;
--- config
    server_tokens off;
    location /t {
        ...
        content_by_lua_block {
            ...
            local offset, err = file:seek("set", -10)
            assert(offset == nil)
            ngx.say(err)
...
invalid argument
Hello, World. I'm just go ahead~
--- grep_error_log: lua io seek whence:0 offset:-10
--- grep_error_log_out
lua io seek whence:0 offset:-10
--- no_error_log eval
["error", "crit"]
